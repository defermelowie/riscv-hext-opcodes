...
def make_rust(instr_dict):
    mask_match_str= '#![allow(dead_code)]\n'
    # Create constants for instructions
    for i in instr_dict:
        mask_match_str += f'pub(crate) const MATCH_{i.upper().replace(".","_")}: u32 = {(instr_dict[i]["match"])};\n'
        mask_match_str += f'pub(crate) const MASK_{i.upper().replace(".","_")}: u32 = {(instr_dict[i]["mask"])};\n'
    # Create constants for CSRs
    for num, name in csrs+csrs32:
        mask_match_str += f'pub(crate) const CSR_{name.upper()}: u16 = {hex(num)};\n'
    # Create name map for CSRs
    mask_match_str += 'pub(crate) fn csr_address_map(csr_name: &str) -> Option<u16> {\n\tmatch csr_name {\n'
    for num, name in csrs+csrs32:
        mask_match_str += f'\t"{name}" => Some(CSR_{name.upper()}),\n'
    mask_match_str += '\t_ => None,\n\t}\n}\n'
    # Create constants for exception causes
    for num, name in causes:
        mask_match_str += f'pub(crate) const CAUSE_{name.upper().replace(" ","_")}: u8 = {hex(num)};\n'
